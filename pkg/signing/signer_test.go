...
func TestNewSigner(t *testing.T) {
	...
	tests := []struct {
		name        string
		provider    signing.PrivateKeyProvider
	...
	}{
		{
			...
			provider:    nil,
			hasher:      &hasherStub{},
			expectError: signing.ErrNilKeyProvider,
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			signer, err := signing.NewSigner(tt.provider, tt.hasher)
...
func TestSignerSign_KeyProviderErrorIsPropagated(t *testing.T) {
	...
	signer, err := signing.NewSigner(provider, nil)
	assert.NoError(t, err)

	tokenStr, err := signer.Sign(context.Background(), []byte("body"))
